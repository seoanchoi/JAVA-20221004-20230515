...
public class Exam10 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < arr1.length; i++) {
			if(arr[i] % 2 == 0) {
				arr1[0] = Arrays.copyOf(arr1[0], arr1[0].length +1);
				arr1[0][arr1[0].length - 1] = arr[i];				
			} else {
				arr1[1] = Arrays.copyOf(arr1[1], arr1[1].length +1);
				arr1[1][arr1[1].length - 1] = arr[i];	
			}
		}
		...
		while(arr1[0].length < 10 || arr1[1].length < 10) {
			...
			if(num % 2 == 0) {
				if(arr1[0].length < 10) {
					arr1[0] = Arrays.copyOf(arr1[0], arr1[0].length +1);
					arr1[0][arr1[0].length - 1] = num;	
				}
			} else {
				if(arr1[1].length < 10) {
					arr1[1] = Arrays.copyOf(arr1[1], arr1[0].length +1);
					arr1[1][arr1[1].length - 1] = num;	
				}
			}
		}
		System.out.println("짝수: " + Arrays.toString(arr1[0]));
		 ...
		 * 		 int idx = num % 2;
		 *  
		 *		if(arr1[inx].length < 10) {
		 *			arr1[inx] = Arrays.copyOf(arr1[inx0], arr1[inx].length +1);
		 *			arr1[inx][arr1[inx].length - 1] = num;	
		 *		}
		 * }
		 * System.out.println("짝수: " + Arrays.toString(arr1[0]));
		 * System.out.println("홀수: " + Arrays.toString(arr1[1]));
