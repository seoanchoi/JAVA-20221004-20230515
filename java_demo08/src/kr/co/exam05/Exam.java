...
public class Exam {
	...
	public static void main(String[] args) {
		...
		if(nowYear == birthYear && nowMonth == birthMonth) {
			...
			dDay += cNow.getMaximum(Calendar.DAY_OF_MONTH) - nowDate;
			while(!(nowYear == birthYear && nowMonth == birthMonth - 1)) {
				cNow.add(Calendar.MONTH, 1);
				nowYear = cNow.get(Calendar.YEAR);
				nowMonth = cNow.get(Calendar.MONTH) + 1;
				dDay += cNow.getMaximum(Calendar.DAY_OF_MONTH);
			}
			dDay += birthDate;
		}
