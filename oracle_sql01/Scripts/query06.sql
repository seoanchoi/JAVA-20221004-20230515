...
SELECT DEPARTMENT_ID
     ...
     , MAX(SALARY) "부서별 최고 급여액"
     , MIN(SALARY) "부서별 처저 급여액"
     , ROUND(AVG(SALARY), 2) "부서별 평균 급여액"
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID;
 
SELECT SUBSTR(PHONE_NUMBER, 1, 3) AS "전화번호 앞자리"
     , COUNT(*) AS 수량
  FROM EMPLOYEES
 ...
 *          그룹으로 묶기 위한 조건이 1개 이상인 경우에 시용하여
 *          그룹별 집계 외에 그룹별 집계 / 복수개의 그룹별 집계 / 총 집계 까지 나타낸다.  
 */
SELECT JOB_ID
	 , COMMISSION_PCT      
	 , COUNT(*) AS 수
...
/*
 * GROUPING : ROLLUP, CUBE 로 집계한 결과에 대해 어떠한 조합으로 그룹을 무껐는지 알 수 있도록
 *           도와주는 함수(SELECT 절에 사용)
